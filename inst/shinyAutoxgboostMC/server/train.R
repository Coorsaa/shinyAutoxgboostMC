...
output$autoxgboos.params.sel = renderUI({
  reqAndAssign(measures.avail(), "ms")
  time.budget = numericInput("time.budget", "Time budget in seconds:", value = 360L, min = 1L)
  iters.budget = numericInput("iter.budget", "Maximum number of Iterations:", value = 20L, min = 1L)
  measures.sel = selectInput("measures.sel", "Measures", choices = ms,
    multiple = TRUE, selected = measures.default())
  run.btn = bsButton("axgb.run", label = "train", icon = icon("graduation-cap"))
  sidebarMenu(
    menuItem("autoxgboost tuning Settings"),
    time.budget,
    iters.budget,
    measures.sel,
    run.btn
...
observeEvent(input$axgb.run, {
  ...
  m = autoxgboostMC::AutoxgboostMC$new(
    task = task(),
    measures = measures.axgb()
  )
...
measures.avail = reactive({
  listMeasures(isolate(task()))
})

measures.default = reactive({
  reqAndAssign(task.type(), "tsk.type")
  switch(tsk.type,
    classif = list("mmce", "f1"),
    regr = "mse")
})

output$measures.sel = renderUI({
  reqAndAssign(measures.avail(), "ms")
  selectInput("measures.sel", "Measures", choices = ms,
    multiple = TRUE, selected = measures.default())
})
